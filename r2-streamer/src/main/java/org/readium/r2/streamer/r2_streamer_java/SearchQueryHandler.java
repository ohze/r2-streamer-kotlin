...
public class SearchQueryHandler extends RouterNanoHTTPD.DefaultHandler {
    ...
    @Override
    public Response get(RouterNanoHTTPD.UriResource uriResource, Map<String, String> urlParams,
        ...
        try {
            Fetcher fetcher = uriResource.initParameter(Fetcher.class);
            ...
            for (Link link : fetcher.getPublication().getSpine()) {
                ...
                String href = link.getHref().substring(1);
                byte[] byteArray = fetcher.data(href);
                String htmlText = new String(byteArray, StandardCharsets.UTF_8);
            ...
            response = Response.newFixedLengthResponse(Status.OK, getMimeType(), json);
            return response;

        } catch (Exception e) {
            Log.e(TAG, "-> get -> ", e);
            return Response.newFixedLengthResponse(Status.INTERNAL_ERROR, getMimeType(),
